...
const BASE_THEME={ bg:'#070b14', paper:'#0d1320', ink:'#e6f0fa', soft:'#9fb3c8', faint:'#5a6b80', line:'#1e2a3d', accent:'#27d3e0' };
...
function trendLabel(t){
  return {up:['📈','主流叙事 · 顺势而为'],hot:['🔥','泡沫狂热 · 风险与暴利并存'],down:['🪐','逆共识 · 多数人看错的方向'],safe:['🛡','现金避险 · 放弃幻想保确定']}[t]||['',''];
}
...
function revealPeriod(){
  ...
  let items=results.map((R,k)=>{
    ...
    <div class="reveal-item" style="animation-delay:${k*0.12}s">
      ...
      <div class="rdeltas">${dc('算力',R.da.aum)}${dc('影响力',R.da.track)}${dc('网络',R.da.net)}</div>
    </div>`;}).join('');
...
function showEnding(healthDead){
  ...
  el.style.setProperty('--ending-bg', meta.bg||'#0d1320');
  el.style.setProperty('--ending-glow', meta.glow||'rgba(39,211,224,.12)');
...
function renderShareQR(){
  try{
    ...
    new QRCode(box, { text:target, width:240, height:240, colorDark:'#0a0e1a', colorLight:'#cfe8f0', correctLevel:QRCode.CorrectLevel.H });
